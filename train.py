...
tf.app.flags.DEFINE_string('train_dir', '/tmp3/fishwei898/mvcnn-tf/tmp_pertb/',
...
def train(dataset_train, dataset_val, ckptfile='', caffemodel=''):
    ...
    with tf.Graph().as_default():
        ...
        prediction2_ = tf.placeholder('int64', shape=(None), name='prediction2_')

        fc8 = model.inference_multiview(view_, keep_prob_)
        loss = model.loss(fc8, y_)
        train_op = model.train(loss, global_step, data_size)
        prediction = model.classify(fc8)   #prediction size = 32 (batch size)
        accuracy = model.accuracy(prediction2_, y_) #y_ size = 256, we need prediction_2 (size = 256) 
        ...
        for epoch in xrange(100):
            ...
            for batch_x, batch_y in dataset_train.batches(batch_size):
                ...
                if step % 100 == 0:# and step > 0:
                    ...
                    acc = sess.run(accuracy, feed_dict={prediction2_: np.array(predictions), y_:val_y[:predictions.size]})
