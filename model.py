...
def _3d_conv(name, in_, ksize, strides=[1,1,1,1,1], padding=DEFAULT_PADDING, reuse=False):

# stddev: standard deviation of normal distribution 
    n_kern = ksize[4]

    with tf.variable_scope(name, reuse=reuse) as scope:
        stddev = 1 / np.prod(ksize[:4], dtype=float) ** 0.5
...
def _fc(name, in_, outsize, wd, dropout=1.0, reuse=False):
    with tf.variable_scope(name, reuse=reuse) as scope:
...
def inference_multiview(views, keep_prob):
    ...
    for i in xrange(V):
        ...
        conv1 = _conv('conv1'+p, view, [11, 11, 3, 96], [1, 4, 4, 1], 'VALID', reuse=reuse)
        lrn1 = None
        pool1 = _maxpool('pool1'+p, conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID')

        conv2 = _conv('conv2'+p, pool1, [5, 5, 96, 256], reuse=reuse)
        lrn2 = None
        pool2 = _maxpool('pool2'+p, conv2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID')
        
        conv3 = _conv('conv3'+p, pool2, [3, 3, 256, 384], reuse=reuse)
        conv4 = _conv('conv4'+p, conv3, [3, 3, 384, 384], reuse=reuse)
        conv5 = _conv('conv5'+p, conv4, [3, 3, 384, 256], reuse=reuse)
    ...
    print 'view_pool', view_pool   
    conv6 = _3d_conv('conv6'+p, view_pool, [12,1,1,256,256], padding="VALID")
    print 'conv6 ', conv6
    fc6 = _fc('fc6', view_pool, 4096, dropout=keep_prob)
    fc7 = _fc('fc7', fc6, 4096, dropout=keep_prob)
    fc8 = _fc('fc8', LSTM_out[-1], 55, wd=0.004)
...
def _view_pool(view_features, name):
    vp = tf.expand_dims(view_features[0], 0) #Inserts a dimension of 1 into a tensor's shape, eg. [100] -> [1, 100]
